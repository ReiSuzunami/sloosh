...
- Windows 11 x86_64 (CLI, daemon, desktop, and Windows Hello helper): `sloosh-windows-x86_64.zip`
...
build the desktop app. The macOS DMG and Windows desktop package are
distributed only through GitHub Releases.
...
1. Detect the OS and architecture. Run `command -v sloosh && sloosh --version`
   on Unix, or `Get-Command sloosh; sloosh --version` in Windows PowerShell.
   ...
   CLI; the Windows ZIP includes both CLI and desktop binaries. Ask before installing anything. Never use `curl | sh`, silently invoke
...
2. Once the CLI works, explain `sloosh init` and ask me to run it in my own
   ...
   myself in its Setup/Security screens for Touch ID or Windows Hello.
...
`target/release/slooshd` (with `.exe` on Windows). See the installation guide
for building the Windows Hello helper.
